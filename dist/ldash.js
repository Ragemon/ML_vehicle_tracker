...
isNode() && (_ = require('lodash'));
/**
 * @description exports inherits
 * @type Object
 */
module.exports = {inherits: inherits};
/**
 * @description wrapper around _.create ,
...
function mid(v1, v2) {
  var dim = 2;
  var boolinput = _.isArray(v1) && _.isArray(v2) &&
  ...
  return _.map(_.zip(v1.slice(0, dim), v2.slice(0, dim)), function (v) {
    return (v[0] + v[1]) / 2.0;
  });
}

/**
 * @description - module exports fixDim
 * @param va{Array}
 * @param vb{Array}
 * @param size{Number}
 * @returns {Array}
 */
function fixDim(va, vb, size) {
  if (_.isUndefined(size)) {
    size = va.length < vb.length ?
           va.length : vb.length;
  }
  return [va.slice(0, size), vb.slice(0, size)]
}

/**
 * @description subGroup a flat array to sub arrays based on step
 * getListSubGroups([0,1,2,3,4,5,6,7],3) === [[0, 1, 2], [3, 4, 5], [6, 7]]
 * @param data
 * @param step
 * @returns {Array}
 */
function subGroup(data, step) {
  return _.map(_.range(0, _.size(data), step), function (i) {
    return data.slice(i, i + step);
  });
}
...
/**
 * @module exports
 * @type {{τ: number, π: number, halfπ: number, ε: number, ε2: number,
 * degrees: number, radians: number, precision: number, sign: Function,
 * acos: Function, asin: Function, sinh: Function, cosh: Function,
 * tanh: Function, haversin: Function, floatequal: Function,
 * hypot: Function, log2: Function, log10: Function, mult2: Function,
 * floorm2: Function, ceilm2: Function, rad2deg: Function, deg2rad: Function,
 * sum: Function, round: Function, trunc: Function, det: Function,
 * detsign: Function, abs: Function}}
 */
module.exports = {
  τ         : τ,
  π         : π,
 ...
 * @description  mult 2
 * @param n
 * @returns {{lg: number, sign: Number}}
 * @private
 */
function mult2(n) {
  "use strict";
  /*
   leftmost or rightmost integer; edge case
   -0.001 ~ -1 and 0.001 ~ 1
   */
  ...
  /*
   check for integer because of small imprecision of representing floats
   eg Math.ceil(31.0000000000000000004) will be 32 instead of 31 (float imprecision)
   */
  lg = floatequal(lg, intlg) ? intlg : lg;
  return {
    lg  : lg,
    sign: sgn
  }
...
/**
 * @description nearest most floor(-) multiple of 2
 * @param n {Number}
 * @returns {Number}
 */
function floorm2(n) {
  ...
  /*
   edge case : floor of less than 2 is 0 , exp ~ 1 or 0
   */
  if ((exp == 0 || exp == 1) && sign > 0 && n < 2) {
    sign = 0
  }
  else if (exp == 0) {
    /*
     make if exponent is 0 then 2^0 == 1 which is not mult 2
     if exp is 0 promote to 1 2^1 == 2
     */
    exp = 1;
  }
  return sign * pow(2, exp);
}

/**
 * @description nearest most ceil(+) multiple of 2
 * @param n {Number}
 * @returns {Number}
 */
function ceilm2(n) {
  ...
  /*
   edge case : ceil of less than -2 is 0 , exp ~ 1 or 0
   */
  if ((exp == 0 || exp == 1) && s < 0 && n > -2) {
    s = 0
  }
  else if (exp == 0) {
    /*
...
/**
 * @description convert deg [,array] to rad
 * @param deg
...
/**
 * @description round number to nearest decimal places
 * @param x
 * @param digits
 * @returns {number}
 */
function round(x, digits) {
  "use strict";
  digits = Math.round(digits) || 0; //convert to integer
  var multiple = pow(10, digits);
  return Math.round(x * multiple) / multiple;
}

/**
 * @description truncates a number to integer
 * @param x {Number}
 * @returns {number}
 */
function trunc(x) {
...
/**
 * @description determinant of 2x2 or 3x3 array
 * @param mat {[[]]}    - multi dimensional array
 * @param dim {number} - dimension
 * @returns {*}
 */
...
/**
 * @description compute hypotenus use _hypot instead of ___hypot
 * javascript has arbitrary precision
 * @param dx{Number}
 * @param dy{Number}
 * @returns {number}
 * @private
 */
function _hypot(dx, dy) {
...
/**
 * @description hypot between two points
 * @param v1
 * @param v2
 * @returns {number}
 * @private
 */
function _hypotpnt(v1, v2) {
  return _hypot(
    parseFloat(v2[0]) - parseFloat(v1[0]),
    parseFloat(v2[1]) - parseFloat(v1[1])
  );
}

/**
 * @description calculates the distance between two points :
 *      v1 and v2 : (x,y) or ('x','y')
 * @param v1{Number|String}
 * @param v2{Number|String}
 * @returns {Number}
 */
function hypot(v1, v2) {
  "use strict";

  var dist;
  var v1_is_array = Array.isArray(v1);
  var v2_is_array = Array.isArray(v2);
  var is_required_args = (arguments.length == 2);

  if (!v1_is_array && typeof !v2_is_array && is_required_args) {
    dist = _hypot(v1, v2);
  }
  else if (v1_is_array && v2_is_array &&
           v1.length && v2.length && (v1.length >= 2 && v2.length >= 2)) {
    dist = _hypotpnt(v1, v2);
  }
  else {
    throw new Error('invalid args');
  }
...
},{"nimble":10}],6:[function(require,module,exports){
/**
 * @module -
 * @description -
 * author: titus
...
function ljust(str, width, pad) {
  ...
  pad = pad || ' ';
  if (str.length < width) {
    return str + repeat(pad, width - str.length);
  }
  else {
    return str;
  }
 ...
 * @returns {*}
 */
function rjust(str, width, pad) {
  "use strict";

  pad = pad || ' ';
  if (str.length < width) {
    return repeat(pad, width - str.length) + str;
  }
  else {
    return str;
  }
...
function center(str, width, pad) {
  ...
  if (str.length < width) {
    ...
    var pads = repeat(pad, parseInt(len / 2));
...
function val(obj, key) {
  "use strict";
  if (arguments.length == 1 || key == undefined
...
},{}],"ldash":[function(require,module,exports){
/**
 * @module - index
...
var mix = {};
ldash.each(mixlist, function (mx) {
  ldash.assign(mix, mx);
});

var constants = ldash.omit(mix, ldash.isFunction);
ldash.each(constants, function (v, k) {
  Object.defineProperty(ldash, k, {writable: false, value: v}); //ldash.prototype[k] = v;
});
ldash.mixin(mix);
 ...
 * @type {*}
 */
module.exports = ldash;


},{"./inherits":1,"./is":2,"./iter":3,"./math":4,"./nimble":5,"./not":6,"./string":7,"./val":8,"lodash":9}]},{},[])
